...
if test -f "${CUSTOM_FILES}/.file_hashes.txt"; then
  mv "${CUSTOM_FILES}/.file_hashes.txt" "${HASH_FILE}"
elif ! test -f "${HASH_FILE}"; then
  touch ${HASH_FILE}
fi
...
if test -f "${CONFIG_FILE}"; then
  ...
  valhalla_build_config --mjolnir-tile-dir ${TILE_DIR} --mjolnir-tile-extract ${TILE_TAR} ${gtfs_dir} ${transit_dir} ${mjolnir_timezone} ${mjolnir_admin} ${additional_data_elevation} ${traffic} > ${CONFIG_FILE}  || exit 1
fi
...
if [[ "${do_admins}" == "True" ]]; then
  ...
  echo "====================="
  echo ""
  valhalla_build_admins --config "${CONFIG_FILE}" ${files}  || exit 1
fi
